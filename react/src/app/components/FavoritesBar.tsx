...
export function FavoritesBar({ onNavigate, currentModule }: FavoritesBarProps) {
  ...
  const favoriteModules = getFavoriteModules(allModules);

  const handleModuleClick = (module: typeof allModules[number]) => {
    ...
    <div className="mb-6">
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <button
          ...
          className="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-all shadow-sm"
        >
          <span>🏠</span>
          <span>Dashboard</span>
        </button>
        ...
        {/* Módulos Favoritos */}
        {favoriteModules.map((module: any) => {
